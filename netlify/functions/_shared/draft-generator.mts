...
export async function runDraftGeneration(input: GenerateDraftInput): Promise<GeneratedDraft> {
  ...
  const { categoryName, categorySlug, contentType, direction, interviewAnswers, businessName, existingArticleTitles } = input;
...
${existingArticleTitles?.length ? existingArticleTitles.map((t) => `- ${t}`).join('\n') : '(none yet — this is the first article for this category)'}
